...
dst_path = "/share/gpu0/jjwhit/mass_map_dataset/kappa20_debug/"
...
for fname in all_files:
    ...
    if (img_number/total_nb_files) <= 0.085:
        dst_dir = dst_train_path
    # Using 10% of data for testing
    elif (img_number/total_nb_files) > 0.085 and (img_number/total_nb_files) <= 0.095:
        dst_dir = dst_test_path
    # Using 5% of data for validation
    elif (img_number/total_nb_files) > 0.095 and (img_number/total_nb_files) <= 0.1:
        dst_dir = dst_val_path
    else:
        continue
